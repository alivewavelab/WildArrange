...
test("decisions and ops panels serve read-only view models", async () => {
  await withTempDir(async (dir) => {
    ...
    await withDashboard(dir, async (base) => {
      ...
      assert.match(page.headers.get("set-cookie"), /^wildarrange_dashboard=[A-Za-z0-9_-]+; HttpOnly; SameSite=Strict; Path=\/$/);
      ...
      assert.match(html, /data-decision-category="routing"/);
      assert.match(html, /data-decision-category="hook"/);
      assert.match(html, /data-decision-category="quality"/);
      assert.match(html, /data-decision-category="admission"/);
      assert.doesNotMatch(html, /data-view="operations"/);
      assert.doesNotMatch(html, /认领任务/);
      assert.doesNotMatch(html, /发送团队消息/);
      assert.match(html, /loadPanels/);
      assert.match(html, /WildArrange 驾驶舱/);
      assert.match(html, /工单总账/);
      assert.match(html, /ledgerTasks/);
      assert.match(html, /ledger-board/);
      assert.match(html, /未开始/);
      assert.match(html, /开发中/);
      assert.match(html, /验收中/);
      assert.match(html, /已通过/);
      assert.doesNotMatch(html, /id="ledgerStatus"/);
      assert.doesNotMatch(html, /taskWorkType/);
      assert.doesNotMatch(html, /API Token/);
      assert.doesNotMatch(html, /saveDashboardToken/);
      assert.match(html, /data-view-panel="overview"/);
      assert.match(html, /当前任务/);
      assert.match(html, /继续推进/);
      assert.match(html, /等待推进/);
      assert.match(html, /正在实现/);
      assert.match(html, /自动检查/);
...
test("route review panel links full prompt, route result, tool activity, and human review", async () => {
  await withTempDir(async (dir) => {
    ...
    await withDashboard(dir, async (base) => {
      ...
      assert.match(html, /质量门/);
      assert.match(html, /成果合入/);
      assert.match(html, /项目治理/);
      assert.match(html, /治理问题整理/);
      assert.match(html, /governanceGrid/);
      assert.match(html, /data-run-category="all"/);
      assert.match(html, /data-run-category="task"/);
      assert.match(html, /data-run-category="quality"/);
      assert.match(html, /data-run-category="admission"/);
      assert.match(html, /data-run-category="recovery"/);
    }, { token: "review-token" });
  });
});

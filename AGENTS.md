...
- `src/infra/context-attachments.mjs`：项目文档与 Skill 的安全读取、预算和内容指纹唯一入口，供 AI 注入与审查复用。
- `src/capabilities/project-review.mjs`：项目级 Review 清单解析、依据加载、独立执行及逐项证据校验；不推进任务状态。
- `src/capabilities/execution-readiness.mjs`：只检查本任务必需的执行者、Skill 与审查依据，返回可运行或明确缺口；不启动业务 Worker、不创建审批记录。

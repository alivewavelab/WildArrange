...
任务声明目标脚本、新增内容、职责前后变化及事实 owner；验收按 R1 已批准范围、R2 独立职责、R3 唯一事实维护者、R4 统一读写入口、R5 无重复实现检查。审查完整受影响脚本，不用行数或 helper 命名直接驳回。退回必须引用具体文件、行号、源码证据和整改方向；声明变更重新人工批准，普通实现修复复用原批准。

### 审查依赖与事实归拢

配置声明项目需要哪些审查依据，能力层装载并验证，编排层只消费结果。旧事实登记保存 owner 与来源引用，不复制业务值；实际消费者迁移通过可验证任务完成。任务意图、审查规范、执行证据分别由现有记录持有，避免第二套批准或完成账本。

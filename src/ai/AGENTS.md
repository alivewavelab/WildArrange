...
- `task.skills` 只在真实接通的执行前公开宿主入口作为可信任务绑定；项目级 Review 使用其清单明确绑定的必需 Skill，完整加载失败时阻止审查；checkpoint 不宣称自动消费任务绑定。任务 Skill 必须经过 Prompt Pack manifest / 项目 Skill 安全解析、安装根/realpath/hash 校验、数量上限和字符预算，不得绕过加载器。
- 截断必须显式报告，不能静默丢失上下文。

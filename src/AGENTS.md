...
- 五区依赖方向以根 `AGENTS.md` 指向的 `doc/code-maintenance.md` 和 `test/dependency-boundary.test.mjs` 为准。
...
2. 修改实现及对应自动测试。
3. 新增跨区边时，先更新 `doc/code-maintenance.md` 中的依赖约束和依赖边界测试，再改实现。
4. 新增运行时模块或改变架构职责时，同步更新 `doc/project-architecture.md`（含「目录约定」），并登记 `tooling/arch-module-graph/module-file-map.json` / `docs/product/architecture-overview.html`。

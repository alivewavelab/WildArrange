...
- 五区实现不得 import `src/helix-core.mjs` 或任何旧 `src/helix-*.mjs` shim。
- 旧 shim 只允许声明式 re-export，不得承载业务逻辑。
...
4. 新增运行时模块或改变架构职责时，同步更新 `doc/project-architecture.md`。
5. 至少运行相关测试；交付前运行 `npm test`。包内容变化时再运行 `npm pack --dry-run --cache /private/tmp/helix-npm-cache`。

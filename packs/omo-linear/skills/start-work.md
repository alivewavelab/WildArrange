...
4. 判断状态：
   - 无 tasks -> 需要导入计划。
   - 有 pending runnable task -> Atlas run。
   - 有 in_progress/verifying task -> 从最新 ledger 证据恢复。
   - 有 failed task -> Sisyphus 决定 retry、Oracle 或用户升级。
...
Atlas 必须处理完整的 worker -> verifier -> checkpoint/retry。不要绕过 Atlas 直接调用 worker。
